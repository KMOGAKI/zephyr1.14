...
static U8 exGpStoragePacketSize(U8 initMode, U8 tstMode)
{
    ...
    gpStorageRef = malloc(A71CH_GP_STORAGE_SIZE);
    gpStorage = malloc(A71CH_GP_STORAGE_SIZE);
    if (!gpStorageRef || !gpStorage) {
        result = 0;
        goto error_packetsize;
    }
...
static U8 exGpStorageFreeze(U8 initMode, U8 tstMode)
{
...
#endif
    U8 *gpStorageRef = NULL;
    U8 *gpStorageNew = NULL;
    U8 *gpStorage = NULL;
    U8 *gpStorageExpected = NULL;
    int i = 0;
    U16 packetSize = 0;
    U16 offset = 0;
    int maxIter = sizeof(testSizes)/sizeof(U16);
    ...
    gpStorageRef = malloc(A71CH_GP_STORAGE_SIZE);
    gpStorageNew = malloc(A71CH_GP_STORAGE_SIZE);
    gpStorage = malloc(A71CH_GP_STORAGE_SIZE);
    gpStorageExpected = malloc(A71CH_GP_STORAGE_SIZE);
    if (!gpStorageRef || !gpStorageNew || !gpStorage || !gpStorageExpected) {
        result = 0;
        goto error_freeze;
    }
...
error_freeze:
    ...
    if (gpStorage)
        free(gpStorage);
    if (gpStorageExpected)
        free(gpStorageExpected);
