...
U16 hcAsnToPlainEcc256(U8 *signature, U16 *pSignatureLen, U8 *asnSignature, U16 asnSignatureLen)
{
    ...
    if (asnSignature[0] == ASN_TAG_SEQUENCE)
    {
        ...
        {
            printk("asnSignature[1] + 2: 0x%02X, asnSignatureLen: 0x%02X", asnSignature[1] + 2, asnSignatureLen); // DEBUG
            *pSignatureLen = 0;
            return ERR_WRONG_RESPONSE;
        }
    }
...
U16 hcNormalizeAsnSignatureEcc(U8 *asnSignature, U16 *asnSignatureLen)
{
    ...
    switch (*asnSignatureLen)
    {
        ...
        default:
            // printk("Skip normalization because of unexpected SignatureLen.\n");
            return ERR_API_ERROR;
    }
    ...
    if (asnSignature[0] == ASN_TAG_SEQUENCE)
    {
        if ( (asnSignature[1]+2) == *asnSignatureLen)
        {
            ...
            for (component=0; component<2; component++)
            {
                // printk("Debug (%s - line:%d): component = %d\n", __FILE__, __LINE__, component);
                if (asnSignature[i] == ASN_TAG_INTEGER)
                {
                    if (asnSignature[i+1] == asnByteCount)
                    {
                        ...
                        {
                            i += 2 + asnSignature[i+1];
                            // printk("asnSignature[i+2] = 0x%02X\n", asnSignature[i+2]);
                            // printk("asnSignature[i+3] = 0x%02X\n", asnSignature[i+3]);
                        }
                    }
                    else
                    {
                        i += 2 + asnSignature[i+1];
                        // printk("asnSignature[i+1] = 0x%02X\n", asnSignature[i+1]);
                    }
                }
                else
                {
#ifdef AX_HC_ASN_LOG_VERBOSE
                    printk("Debug (%s - line:%d): Failed to parse ASN_ECDSA_SIGNATURE.\n", __FILE__, __LINE__);
#endif
                    *asnSignatureLen = 0;
                    return ERR_WRONG_RESPONSE;
                }
