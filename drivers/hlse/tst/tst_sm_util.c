...
int axPrintByteArray(const char *pName, const U8 *pData, U16 dataLength, U16 style)
{
    ...
    printk("%s (LEN=%d):%c", pName, dataLength, (nBreak > 0) ? '\n' : ' ');
    ...
    for (i = 0; i < dataLength ; i++)
    {
        if (i == (dataLength-1))
        {
            ...
            printk("%s%02X", preDeco, pData[i]);
        }
        else
        {
            printk("%s%02X%s", preDeco, pData[i], postDeco);
        }
        if (nBreak > 0 )
        {
            if ((i+1) % nBreak == 0) {printk("\n");}
        }
    }
    printk("\n");
#endif
...
#define BIN2HEX(n) (((n) <= 9) ? ((n) + '0') : ((n) - 10 + 'A'))
/**
...
int axConvertByteArray2HexString(char *string, int stringBufSize, const U8 *byteArray, int nByte, U16 style)
{
    ...
    for (i = 0; i < nByte; i++)
    {
        string[ i << 1     ] = BIN2HEX(byteArray[i] >> 4);
        string[(i << 1) + 1] = BIN2HEX(byteArray[i] & 0xf);
    }
    string[i << 1] = '\0';
...
int axConvertHexString2ByteArray(U8 *byteArray, const char *string, int nOffset, int nByte)
{
    ...
    for (j=0; j<nByte; j++) {
        ...
        if (pastConverted == byteAsString) {
            // Conversion failed
            printk("(%s/%d) %s can not be converted to HEX value.\n", __FILE__, __LINE__, byteAsString);
            return AX_UTIL_ERROR;
        }
...
U8 axCheckSw(U16 sw, U16 expectedSw, char *msg, char *szFilename, int lineNr)
{
    if (sw != expectedSw)
    {
        printk("\n***** ERROR (%s)\n", msg);
        printk("%s: line=%d\n", szFilename, lineNr);
        printk("***** Expected SW 0x%04x, but got 0x%04X\n", expectedSw, sw);
        return 0;
    }
    else
    {
...
U8 axCheckU8(U8 in, U8 expected, char *msg, char *szFilename, int lineNr)
{
    if (in != expected)
    {
        printk("\n***** ERROR (%s)\n", msg);
        printk("%s: line=%d\n", szFilename, lineNr);
        printk("***** Expected 0x%02x, but got 0x%02x\n", expected, in);
        return 0;
    }
    else
    {
...
U8 checkBytestring(U8 *pA, U16 aLength, U8 * pB, U16 bLength, char *msg)
{
    ...
    {
        printk("\n***** ERROR (%s)\n", msg);
        printk("Bytestring are different:\n");
...
U8 checkErr(U16 err, U16 expectedErr, char *msg)
{
    ...
    {
        printk("\n***** ERROR (%s)\n", msg);
        printk("***** Expected ERR 0x%04x, but got 0x%04X\n", expectedErr, err);
        // assert(0);
        return 0;
    }
    else
...
U8 checkU16(U16 in, U16 expected, char *msg)
{
    ...
    {
        printk("\n***** ERROR (%s)\n", msg);
        printk("***** Expected 0x%04x, but got 0x%04x\n", expected, in);
        // assert(0);
        return 0;
    }
    else
